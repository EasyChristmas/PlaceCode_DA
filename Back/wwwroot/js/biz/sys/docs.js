$(function () { 
    $.tableHeight(72, ".menu");
    $.tableHeight(72, "iframe");
    //左侧菜单json
    var j_mJson = '';    $.ajax({
        url: "/js/biz/docsdata.json?_=" + Math.random(),
        type: "GET",
        dataType: "json",
        async: false,
    ...
    //默认第一个展开
    $('.jMenuTree>li').eq(0).find('.sub_menu_1').show();
    $('.jMenuTree>li').eq(0).addClass('on');
    $('.jMenuTree>li').eq(0).find('.sub_li_2').eq(0).addClass('selected');
});
