  ...
  Class.prototype.upload = function(files, type){
    ...
    ,ajaxSend = function(){
      ...
      layui.each(items, function(index, file){
        ...
        //正在上传样式处理
        var loading = $(elemFile).parent(), defaultIcon = loading.text();
        loading.html(loading.html().replace(defaultIcon, "&#xe63d;"));
        loading.addClass("layui-anim layui-anim-rotate layui-anim-loop");
        $(elemFile).prop("disabled","disabled");
        //提交文件
        $.ajax({
          url: options.url
          ,type: options.method
          ...
          ,success: function(res){
            loading.removeClass("layui-anim layui-anim-rotate layui-anim-loop");
            loading.html(loading.html().replace(loading.text(),defaultIcon));
            $(elemFile).prop("disabled","");
            successful++;
            done(index, res);
            allDone();
          }
